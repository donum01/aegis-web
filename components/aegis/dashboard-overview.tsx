...
export function DashboardOverview({ summary }: { summary: DashboardSummary }) {
  ...
  return (
    <section className="aegis-card overflow-hidden" aria-labelledby="financial-position-heading">
      <div className="p-5 sm:p-7">
        <p id="financial-position-heading" className="aegis-eyebrow">Net account value</p>
      ...
      <dl className="grid grid-cols-1 border-t sm:grid-cols-3" style={{ borderColor: "var(--aegis-card-border)" }}>
        {details.map((item, index) => (
          <div
            key={item.label}
            className={`px-5 py-4 sm:px-6 ${index > 0 ? "border-t sm:border-l sm:border-t-0" : ""}`}
            style={{ borderColor: "var(--aegis-card-border)" }}
          >
            <dt className="text-xs font-medium" style={{ color: "var(--aegis-muted)" }}>{item.label}</dt>
            <dd className="mt-1 text-lg font-semibold tabular-nums">{item.value}</dd>
            <p className="mt-1 text-xs" style={{ color: "var(--aegis-muted)" }}>{item.detail}</p>
          </div>
        ))}
      </dl>
    </section>
